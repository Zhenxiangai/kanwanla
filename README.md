...
| 这次更新 | 对你的好处 |
| --- | --- |
| 打开插件后自动开始概览 | 可以先阅读字幕，章节和重点摘录会同时在后台生成。 |
| 切换页面后返回会续接任务 | 不再从头生成，也不会因为重新打开侧栏而重复调用模型。 |
| 恢复真实处理阶段和已用时间 | 返回原视频时能看见同一任务的最新进度。 |
| 更新按钮增加侧栏直接回退 | Chrome 旧后台异常时仍可检查并打开可信的最新版下载页。 |

以前几个版本也解决了很实际的问题：v2.2.2 统一国内仓库，v2.1.3 增加国内下载，v2.1.2 把检查更新放到侧栏顶端，v2.1.1 修复笔记并加入划线提问、真实进度和 Agent 导出。
